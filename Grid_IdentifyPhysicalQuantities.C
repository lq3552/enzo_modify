...
int grid::IdentifyPhysicalQuantities(int &DensNum, int &GENum, int &Vel1Num,
				     int &Vel2Num, int &Vel3Num, int &TENum, int &G0Num, int &ZetaNum)
{
 
  DensNum = GENum = Vel1Num = Vel2Num = Vel3Num = TENum = G0Num = ZetaNum = 0;
  ...
  if ((G0Num = FindField(G0, FieldType, NumberOfBaryonFields)) < 0) {
      ENZO_FAIL("Cannot find G0.");
  }

  /* Find Zeta, if possible */
 
  if ((ZetaNum = FindField(Zeta, FieldType, NumberOfBaryonFields)) < 0) {
      ENZO_FAIL("Cannot find Zeta.");
  }
